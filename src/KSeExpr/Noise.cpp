...
template<int d, class T, bool periodic> T noiseHelper(const T *X, const int *period = 0)
{
    // find lattice index
    T weights[2][d]; // lower and upper weights
    int index[d];
    for (int k = 0; k < d; k++) {
        T f = KSeExpr::Utils::floor(X[k]);
        index[k] = (int)f;
        if (periodic) {
            index[k] %= period[k];
            if (index[k] < 0)
...
template<int d_in, int d_out, class T> void CellNoise(const T *in, T *out)
{
    ...
    for (int k = 0; k < d_in; k++)
        index[k] = uint32_t(KSeExpr::Utils::floor(in[k]));
    while (1) {
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 10000000; i++) {
        ...
        sum += KSeExpr::noiseHelper<3, T, false>(foo);
    }
}
#endif
