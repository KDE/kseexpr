...
"""
Helpful utilities that make using the KSeExprPy syntax traversal class easier
"""
...
def traverseFilter(node,filterCallback):
    ...
    For example:
        expr='a+3+map("test",f())'
        ast=KSeExprPy.AST(expr)
        allCalls=KSeExprPy.traverseFilter(ast.root(),lambda node,children: node.type==KSeExprPy.ASTType.Call)
        allMapNodes=KSeExprPy.traverseFilter(ast.root(),lambda node,children: node.type==KSeExprPy.ASTType.Call and node.value=="map")
        allVarRefs=KSeExprPy.traverseFilter(ast.root(),lambda node,children: node.type==KSeExprPy.ASTType.Var)
    """
    ret=[]
...
class Edits:
    ...
    For example:
        ...
        ast=KSeExprPy.AST(originalExpression)
        edits=KSeExprPy.Edits(originalExpression)
        literals=[]
        def doubleAllNumberLiterals(node,children):
            if node.type==KSeExprPy.ASTType.Num: edits.addEdit(node,node.value*2)
        KSeExprPy.traverseCallback(ast.root(),doubleAllNumberLiterals)
