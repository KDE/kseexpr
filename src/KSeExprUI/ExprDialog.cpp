...
void ExprDialog::setupHelp(QTabWidget* tab) {
    ...
    QFile* helpDoc = new QFile(QString::fromLatin1("%1/../share/doc/KSeExpr/SeExpressions.html").arg(QCoreApplication::applicationDirPath()));
    if (helpDoc->exists()) {
        QString sheet =
...
void ExprDialog::applyExpression() {
    ...
    if (!valid) {
        const std::vector<KSeExpr::Expression::Error>& errors = grapher->expr.getErrors();
