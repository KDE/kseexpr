...
apps.each do |app|
    ...
    if appdata["gitModule"]
        ...
        puts "-> Fetching https://invent.kde.org/#{appdata["category"]}/#{appdata["name"]}/-/archive/#{appdata["gitTag"]}/#{appdata["name"]}-#{appdata["gitTag"]}.tar.gz"
    else
    ...
    if appdata["gitModule"]
        `curl "https://invent.kde.org/#{appdata["category"]}/#{appdata["name"]}/-/archive/#{appdata["gitTag"]}/#{appdata["name"]}-#{appdata["gitTag"]}.tar.gz" | tar xz --strip-components=1`
    else
        if appdata["wholeModule"]
            `svn co #{svnroot}/#{appdata["mainmodule"]}/#{appdata["submodulepath"]} #{rev} #{appdata["name"]}-tmp`
        else
            `svn co #{svnroot}/#{appdata["mainmodule"]}/#{appdata["submodulepath"]}#{appdata["name"]} #{rev} #{appdata["name"]}-tmp`
        end
        Dir.chdir( appdata["name"] + "-tmp" )

        if appdata["docs"] != "no"
            if !appdata["docpath"]
                temp = { "docpath" => "doc/#{appdata["name"]}" }
                appdata = appdata.merge(temp)
            end
